...
abstract class SoftWayLoader
{
    ...
    public static function import($key, $base = null)
    {
        ...
        if (!isset(self::$imported[$key]))
        {
            ...
            if(class_exists($class) && array_key_exists($key,self::$imported)){
    ...
    public static function load($class)
    {
        ...
        if (isset(self::$classes[$key]) && file_exists(self::$classes[$key]))
        {
    ...
    public static function registerPrefix($prefix, $path, $reset = false, $prepend = false)
    {
        ...
        if (!file_exists($path))
        {
            $path = (str_replace(EASY_BK_APPOINTMENT_PATH_ROOT, '', $path) == $path) ? basename($path) : str_replace(EASY_BK_APPOINTMENT_PATH_ROOT, '', $path);
