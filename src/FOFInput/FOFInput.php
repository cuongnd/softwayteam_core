...
class FOFInput extends Input
{
	...
	public function __construct($source = null, array $options = array())
	{
		...
		// Magic quotes GPC handling (something Input simply can't handle at all)
    ...
    public static function &getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1, $stripUSC = -1)
    {
        ...
        {
            self::$instances[$sig] = new FOFInput($tagsArray, $attrArray);
        }
	...
	public static function __callStatic($name, $arguments)
	{
		...
		if (substr($name, 0, 3) == 'get')
		{
			...
			if (!($input instanceof FOFInput) && !($input instanceof Input))
			{
