...
?><?php
class WBPaymentUserpoints extends WBPayment {
	var $multiple = true;
	var $name = 'userpoints';
	...
	function checkPaymentDisplay(&$method, &$order) {
		...
		if(!empty($method->payment_params->virtual_coupon)) {
			...
			$no_virtual_coupon = (int)$app->getUserState(HIKASHOP_COMPONENT.'.userpoints_no_virtual_coupon', (@$opt['checkout_step'] && @$opt['default_no_use']) );
			...
			if($old_discount != $new_discount) {
				...
				$order->additional['userpoints']->price_value_with_tax = $currencyClass->getTaxedPrice($order->additional['userpoints']->price_value, $zone_id, $tax_id);
				$order->additional['userpoints']->price_currency_id = $currency_id;
	...
	function finalPriceToPoints(&$order, &$userPoints, $additional = true) {
		if(empty($this->plugin_params))
			return 0;
		...
		$rounding = 0;
		if(!empty($this->plugin_params->rounding) && (int)$this->plugin_params->rounding > 0)
			$rounding = (int)$this->plugin_params->rounding;
