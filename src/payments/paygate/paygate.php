...
?><?php
class WBPaymentPaygate extends WBPayment
{
	var $multiple = true;
	var $name = 'paygate';
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id) //On the checkout
	{
		...
		{
			$this->app->enqueueMessage(WoobookingText::sprintf('CONFIGURE_X_PAYMENT_PLUGIN_ERROR','an identifer','Paygate'),'error');
			return false;
		}
		elseif (empty($this->payment_params->key))
		{
			$this->app->enqueueMessage(WoobookingText::sprintf('CONFIGURE_X_PAYMENT_PLUGIN_ERROR','a key','Paygate'),'error');
			return false;
		}
		elseif (empty($this->payment_params->payment_url))
		{
			$this->app->enqueueMessage(WoobookingText::sprintf('CONFIGURE_X_PAYMENT_PLUGIN_ERROR','a payment url','Paygate'),'error');
			return false;
		}
		else
		{
	...
	function onPaymentNotification(&$statuses)
	{
		...
		elseif($vars['TRANSACTION_STATUS']!='1' or $vars['RESULT_CODE']!='990017')
		{
			...
			$this->modifyOrder($order_id, $this->payment_params->invalid_status, true, true);
			$this->app->enqueueMessage(WoobookingText::sprintf('TRANSACTION_PROCESSING_ERROR','Invalid Paygate\' transaction status'),'error');
			$this->app->redirect($cancel_url);
			return false;
		}
		else
