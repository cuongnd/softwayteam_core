...
?><?php
class WBPaymentVnPaygate extends WBPayment
{
	var $multiple = true;
	var $name = 'vnpaygate';
	var $doc_form = 'vnpaygate';
	...
	{
		$element->payment_name='vnpaygate';
		$element->payment_description='You can pay by credit card using this payment method';
		$element->payment_images='MasterCard,VISA,Credit_card,American_Express';
		$element->payment_params->invalid_status='cancelled';
		$element->payment_params->verified_status='confirmed';
		$element->payment_params->payment_url = 'https://www.vnpaygate.co.za/PayWebv2/process.trans';
	}
	...
	function paygate_signature($pwd, $parameters, $debug=false, $decode=false)
	{
		...
		{
			$this->app->enqueueMessage('The vnpaygate payment plugin requires at least the PHP 5.1.2 version to work, but it seems that it is not available on your server. Please contact your web hosting to set it up.','error');
			return false;
		}
		else
		{
