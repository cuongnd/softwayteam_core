...
?><?php
class WBPaymentPaypalAdaptive extends WBPayment
{
	...
	public function onPaymentNotification(&$statuses) {
		...
		if(JRequest::getCmd('mode', '') == 'popup' && JRequest::getCmd('subtask', '') != '') {
			$order_id = (int)@$_GET['order_id'];
			$task = JRequest::getCmd('subtask', '');
...
window.hikashop.ready(function() {
	...
	top.close();
		...
		if(!empty($suborders) && empty($this->payment_params->classical)) {
			...
			if(!empty($suborders_id)) {
				$query = 'UPDATE ' .  hikashop_table('order') . ' SET order_vendor_paid = order_id WHERE order_id IN (' . implode(',', $suborders_id) . ')';
				$db->setQuery($query);
				$db->query();
			}
		}
	...
	private function sendIPNconfirm($notif_url, $data = '') {
		if(!empty($this->payment_params->use_fsock))
			return $this->sendIPNconfirm_socket($notif_url, $data);

		return $this->sendIPNconfirm_socket($notif_url, $data);
	}

	private function sendIPNconfirm_socket($notif_url, $data = '') {
