...
?><?php
class WBPaymentMangopay extends WBPayment
{
	...
	public function onAfterOrderConfirm(&$order, &$methods, $method_id) {
		...
		if($this->payment_params->payment_mode == 'direct') {
			...
			$cardRegister->Currency = $this->currency->currency_code;
	...
	protected function paymentNotificationCardReturn($order_id, &$statuses) {
		...
		$payIn->DebitedFunds->Currency = $this->currency->currency_code;
		$payIn->Fees = new MangoPay\Money();
		$payIn->Fees->Amount = 0;
		$payIn->Fees->Currency = $this->currency->currency_code;
	...
	protected function payVendors(&$order) {
		...
		if(empty($order->order_payment_params->mangopay->walletId))
			return false;
		...
		if(!empty($suborders)) {
			...
			foreach($suborders as $k => $suborder) {
				...
				if($price < 0)
					$price = round(hikamarket::toFloat($suborder->order_vendor_price) + $order->cart->full_total->prices[0]->price_value_with_tax, (int)$this->currency->currency_locale['int_frac_digits']);
			...
			foreach($vendors as $k => $v) {
				$this->transfert(
					...
					$order->order_payment_params->mangopay->walletId,
					$v['wallet'],
					$v['full_price'],
		...
		$order_payment_params = hikamarket::cloning($order->order_payment_params);
		unset($order_payment_params->mangopay->walletId);
		$order_payment_params->mangopay->paid = true;

		$query = 'UPDATE ' .  hikashop_table('order') .
				' SET order_payment_params = ' . $db->Quote(serialize($order_payment_params)) .
				' WHERE order_id = ' . (int)$order->order_id;
		$db->setQuery($query);
		$db->query();

		if(!empty($suborders_id)) {
			$query = 'UPDATE ' .  hikashop_table('order') .
