 ...
 * @since  1.5
 */
abstract class HtmlFrontendMenu
{
	/**
	...
	public static function menuItems($config = array())
	{
		...
		if (empty(static::$items[$key]))
		{
			...
			foreach ($menus as &$menu)
			{
				...
				static::$items[$key][] = HtmlFrontend::_('select.option', $menu->value . '.1', SoftWayText::_('JLIB_HTML_ADD_TO_THIS_MENU'));
	...
	public static function linkOptions($all = false, $unassigned = false, $clientId = 0)
	{
		...
		if ($all | $unassigned)
		{
			$mitems[] = HtmlFrontend::_('select.option', '<OPTGROUP>', SoftWayText::_('JOPTION_MENUS'));

			if ($all)
			{
				$mitems[] = HtmlFrontend::_('select.option', 0, SoftWayText::_('JALL'));
			}

			if ($unassigned)
			{
				$mitems[] = HtmlFrontend::_('select.option', -1, SoftWayText::_('JOPTION_UNASSIGNED'));
			}
