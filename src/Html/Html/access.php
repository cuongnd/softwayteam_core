...
abstract class HtmlAccess
{
	...
	public static function level($name, $selected, $attribs = '', $params = true, $id = false)
	{
		...
		elseif ($params)
		{
			array_unshift($options, Html::_('select.option', '', SoftWayText::_('JOPTION_ACCESS_SHOW_ALL_LEVELS')));
		}
	...
	public static function usergroup($name, $selected, $attribs = '', $allowAll = true, $id = false)
	{
		...
		if ($allowAll)
		{
			array_unshift($options, Html::_('select.option', '', SoftWayText::_('JOPTION_ACCESS_SHOW_ALL_GROUPS')));
		}
	...
	public static function usergroups($name, $selected, $checkSuperAdmin = false)
	{
		...
		for ($i = 0, $n = count($groups); $i < $n; $i++)
		{
			...
			if ((!$checkSuperAdmin) || $isSuperAdmin || (!JAccess::checkGroup($item->id, 'core.admin')))
			{
				...
				$html[] = '					' . $checked . $rel . ' />';
				$html[] = '			' . JLayoutHelper::render('SoftWay.html.treeprefix', array('level' => $item->level + 1)) . $item->title;
				$html[] = '			</label>';
				$html[] = '		</div>';
				$html[] = '	</div>';
			}
