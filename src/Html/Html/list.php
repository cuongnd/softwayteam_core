...
use SoftWay\String\StringHelper;
...
abstract class HtmlList
{
	...
	public static function images($name, $active = null, $javascript = null, $directory = null, $extensions = 'bmp|gif|jpg|png')
	{
		...
		$images = array(Html::_('select.option', '', SoftWayText::_('JOPTION_SELECT_IMAGE')));
	...
	public static function genericordering($query, $chop = 30)
	{
		...
		if (empty($items))
		{
			$options[] = Html::_('select.option', 1, SoftWayText::_('JOPTION_ORDER_FIRST'));

			return $options;
		}

		$options[] = Html::_('select.option', 0, '0 ' . SoftWayText::_('JOPTION_ORDER_FIRST'));
		...
		$options[] = Html::_('select.option', $items[$i - 1]->value + 1, ($items[$i - 1]->value + 1) . ' ' . SoftWayText::_('JOPTION_ORDER_LAST'));
	...
	public static function users($name, $active, $nouser = 0, $javascript = null, $order = 'name')
	{
		...
		if ($nouser)
		{
			$users[] = Html::_('select.option', '0', SoftWayText::_('JOPTION_NO_USER'));
