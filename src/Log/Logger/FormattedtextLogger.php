...
/**
 * softwaycore! Formatted Text File Log class
 *
...
class FormattedtextLogger extends Logger
{
	...
	public function addEntry(LogEntry $entry)
	{
		...
		else
		{
			...
			if (!File::append(SOFT_WAY_CORE_PATH_ROOT.$this->path, $line))
			{
				throw new \RuntimeException('Cannot write to log file.');
			}
		}
	...
	protected function initFile()
	{
		...
		Folder::create(SOFT_WAY_CORE_PATH_ROOT.dirname($this->path));

		// Build the log file header.
		$head = $this->generateFileHeader();

		if (!File::write(SOFT_WAY_CORE_PATH_ROOT.$this->path, $head))
		{
			throw new \RuntimeException('Cannot write to log file.');
		}
	}
