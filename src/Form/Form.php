...
/**
 * Form Class for the softwaycore Platform.
 *
...
class Form
{
	 ...
	 * the form control is set to 'softwaycore' however, the fields would be rendered like:
	 * `<input name="softwaycore[foo]" />` and `<input name="softwaycore[bar]" />`.
	 *
	...
	protected function syncPaths()
	{
		...
		{
			$path = SOFT_WAY_CORE_PATH_ROOT . '/' . ltrim($path, '/\\');
	 ...
	 */
	public static function getInstance($data = null, $options = array(), $replace = true, $xpath = false)
	{

		// Reference to array with form instances
		$form = &self::$instance[$data];
		...
		if (!$form)
		{
			...
			if (empty($data))
			{
				throw new \InvalidArgumentException(sprintf('%1$s( *%3$s*)', __METHOD__, gettype($data)));
			}

			// Instantiate the form.
            self::$instance[$data] = new static($options);
